...
def find_file(filename, obj, clientfilename, cli, db, fs, flog):#filename是文件在数据库里的名字，obj是前端发来的文件相关信息
...
def list_all_shapes(cli, db, fs, flog):  # 查找所有外形名（未考虑没有数据的情况）
    name_arr = []  # 定义外形名字数组
...
def list_doc_tree(name, cli, db, fs, flog):  # 根据外形名查找对应的目录树，若没有则返回null
    doc_tree = 'null'
...
def check_user(name, password, cli, db, fs, flog):  # 验证用户名密码是否正确（未考虑用户名不存在）
...
def add_user(name, password, authority, cli, db, fs, flog):  # 前端管理员添加用户
...
def del_user(name, cli, db, fs, flog):  # 前端管理员删除用户
...
def count_users(cli, db, fs, flog):  # 计算用户数量
    users_num = db.USERS.find().count()
    return users_num


def get_user_list(cli, db, fs, flog):  # 获得所有用户信息列表
    user_arr = []
