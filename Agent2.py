...
def handle_req(conn, req):  # 处理前端请求
    if req['type'] == 'list_all_shapes':  # 前端向后台发送请求，获取最外层目录
        ...
        fp = open('req_ALEX_20170708142032_list_all_shapes.json', 'w')
        fp.write(str(response))

        fp.close()
        t = tarfile.open("req_ALEX_20170708142032_list_all_shapes.tar.gz", "w:gz")
        t.add("req_ALEX_20170708142032_list_all_shapes.json")
        t.close()
        t = open('req_ALEX_20170708142032_list_all_shapes.tar.gz', 'rb')
        while 1:
        ...
        if req['ack'] == 1:
            print(conn.send(doc_tree.encode('utf-8')))  # 后台向前端发送内层目录
        else:
            response = {
                "type": "list_doc_tree",
                "doc_tree_length": len(doc_tree),
                "doc_tree_name": req['doc_tree_name']
            }
            send_j_response(conn, response)
...
def send_j_response(conn, res):  # 向前台发送json
    j_res = json.dumps(res)
    conn.sendall(str(j_res).encode('utf-8'))
    print(res)
